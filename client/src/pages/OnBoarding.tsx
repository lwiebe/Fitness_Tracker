      ...
      <div className="onboarding-container">
        ...
        <div className="px-6 mb-8 onboarding-wrapper">
          <div className="flex gap-2 max-w-2xl">
            {[1, 2, 3].map((s) => (
              <div key={s} className={`h-1.5 flex-1 rounded-full transition-all duration-500 ${s <= step ? "bg-emerald-500" : "bg-slate-200 dark:bg-slate-800"}`} />
            ))}
          </div>
          ...
          {step === 2 && (
            <div className="space-y-6 onboarding-wrapper">
              ...
              <div className="flex flex-col gap-4 max-w-2xl">
                ...
                <Input label="Weight" type="number" value={formData.weight} onChange={(v) => updateField('weight', v)} placeholder="Enter your weight" min={20} max={300} required />
          ...
          {step === 3 && (
            <div className="space-y-6 onboarding-wrapper">
              ...
              <div className="space-y-8 max-w-lg">
                ...
                <div className="space-y-6">
                  <Slider label="Daily Calorie Intake" min={120} max={4000} step={50} value={formData.dailyCaloriesIntake} onChange={(v) => updateField('dailyCaloriesIntake', v)} unit="kcal" infoText="The total calories you plan to consume each day." />

                  <Slider label="Daily Calorie Burn" min={100} max={2000} step={50} value={formData.dailyCaloriesBurn} onChange={(v) => updateField('dailyCaloriesBurn', v)} unit="kcal" infoText="The total calories you aim to burn through exercise and activity each day." />
                </div>
              </div>
            </div>
          )}
